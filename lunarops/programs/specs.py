...
OBSERVATION_FIELDS = (
    ...
    class_config("stationDisplacement", "stationDisplacement"),
...
def observation_fields(
    *,
    ...
    if adjustment:
        fields.extend(ADJUSTMENT_FIELDS)
    if residual:
        fields.extend(RESIDUAL_FIELDS)
    return tuple(fields)


def validate_adjustment_config(config: dict, path: str) -> dict:
    """Run the scientific adjustment parser as the program schema validator."""
